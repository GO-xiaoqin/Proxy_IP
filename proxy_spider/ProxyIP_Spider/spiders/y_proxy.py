...
class ProxySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in proxy_list:
            ...
            proxy_type = i.xpath('.//td[4]/text()').get()
            proxy_response_speed = i.xpath('.//td[6]/text()').get()
            proxy_verification_time = i.xpath('.//td[last()]/text()').re(r'\d+/\d+/\d+')
            # 进行过滤筛选
            if not all([
        ...
        if proxy_item and time.strptime(proxy_item['proxy_verification_time'][0], "%Y/%m/%d") == today:
            next_page = response.xpath('//*[@id="listnav"]/ul/a[contains(text(),"下一页")]/@href').get()
